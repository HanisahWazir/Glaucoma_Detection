...
# Inspect the dataset structure and class distribution
df.info()
print(df.head())
...
# Create a patient-level dataset so each patient is assigned to exactly one train, validation, or test partition
patient_df = df[["Patient", "Label Encode"]].drop_duplicates(subset="Patient")

# Split patients into 70% training and 30% temporary data.
# Patient-level splitting prevents data leakage from multiple images belonging to the same patient.
...
# Split temporary patients into 10% validation and 20% test
val_patients, test_patients = train_test_split(
    temp_patients,
    test_size = 2 / 3,
    random_state = 1234,
    stratify = temp_patients["Label Encode"] 
)

# Assign all images belonging to each patient to their respective split
...
# Convert image paths to strings (tensorflow needs it as strings)
...
# Save the exact splits
(PROJECT_ROOT / "data" / "splits").mkdir(parents=True, exist_ok=True)
train_df.to_csv(PROJECT_ROOT / "data" / "splits" / "train_split.csv", index=False)
val_df.to_csv(PROJECT_ROOT / "data" / "splits" / "validation_split.csv", index=False)
test_df.to_csv(PROJECT_ROOT / "data" / "splits" / "test_split.csv", index=False)

print("\nSaved the train, validation and test splits.")
...
# Calculate mean and standard deviation of image quality scores for the overall dataset and each data partition
...
# Apply ResNet-50-specific preprocessing and load images in batches during model training and evaluation
...
train_generator = train_datagen.flow_from_dataframe(
    ...
    class_mode="raw",   # since Label Encode is already 0/1
    shuffle=True,
    seed = SEED
)
...
# ============================================================
# 8. BUILD THE RESNET-50 BASELINE MODEL
# ============================================================

# Load the ImageNet-pretrained ResNet-50 convolutional backbone
base_model = ResNet50(
    weights="imagenet",
    include_top=False,
...
# Build binary glaucoma classifier
model = Sequential([
    base_model,
...
# Save this exact trained model
PROJECT_ROOT.mkdir(parents=True, exist_ok=True)
model.save(PROJECT_ROOT / "models" / "resnet50_baseline.keras")
...
# More detailed evaluation using sklearn (since you already imported these)
...
# Save predictions and probabilities for each test image to support further analysis, uncertainty evaluation and error investigation
...
baseline_results_df.to_csv(
    PROJECT_ROOT / "results" / "baseline" / "resnet50_baseline_results.csv",
    index=False
)

print(
    "\nSaved image-level baseline results to "
    "'resnet50_baseline_results.csv'."
)