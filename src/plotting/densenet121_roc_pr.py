...
# ============================================================
# 3. LOAD DENSENET-121 BASELINE RESULTS
# ============================================================

# Load the previously saved DenseNet-121 baseline results
df = pd.read_csv(PROJECT_ROOT / "results" / "baseline" / "densenet121_baseline_results.csv")

# Extract the true labels and predicted probabilities
y_true = df["True Label"].to_numpy()
y_prob = df["Baseline Probability"].to_numpy()
...
axes[0].plot(
    ...
    tpr,
    linewidth=2,
    label=f"DenseNet-121 (AUROC = {auroc:.4f})"
)
...
axes[0].set_title("(a) ROC Curve for DenseNet-121 Baseline")
...
axes[1].plot(
    ...
    precision,
    linewidth=2,
    label=f"DenseNet-121 (AUPRC = {auprc:.4f})"
)
...
axes[1].set_title("(b) Precision-Recall Curve for DenseNet-121 Baseline")
...
plt.savefig(
    PROJECT_ROOT / "figures" / "figure3.png",
    dpi=300,
    bbox_inches="tight"
)
