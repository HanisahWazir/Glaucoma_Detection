...
# ============================================================
# 3. LOAD RESNET-50 MC DROPOUT RESULTS
# ============================================================

# Load the previously saved ResNet-50 MC Dropout results
df = pd.read_csv(PROJECT_ROOT/ "results" / "mc_dropout" / "resnet50_mc_dropout_results.csv")

# Extract the true labels
y_true = df["True Label"].to_numpy()

# Extract the mean predicted probabilities from MC Dropout
y_prob = df["MC Mean Probability"].to_numpy()
...
axes[0].plot(
    ...
    tpr,
    linewidth=2,
    label=f"ResNet-50 + MC Dropout (AUROC = {auroc:.4f})"
)
...
axes[0].set_title("(a) ROC Curve for ResNet-50 + MC Dropout")
...
axes[1].plot(
    ...
    precision,
    linewidth=2,
    label=f"ResNet-50 + MC Dropout (AUPRC = {auprc:.4f})"
)
...
axes[1].set_title("(b) Precision-Recall Curve for ResNet-50 + MC Dropout")
...
plt.savefig(
    PROJECT_ROOT / "figures" / "figure5.png",
    dpi=300,
    bbox_inches="tight"
)
