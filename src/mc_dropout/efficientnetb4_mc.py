...
test_df["Image Path"] = test_df["Image Path"].astype(str).apply(lambda p: str((PROJECT_ROOT / p).resolve()) if not Path(p).is_absolute() else p)
...
# EfficientNetB4 includes its own input preprocessing
...
# ============================================================
# 6. LOAD THE TRAINED EFFICIENTNETB4 MODEL
# ============================================================

# Load the previously trained EfficientNetB4 baseline model
model = tf.keras.models.load_model(PROJECT_ROOT / "models" / "efficientnetb4_baseline.keras")

print("\nLoaded the saved EfficientNetB4 baseline model.")
model.summary()
...
mc_results_df.to_csv(
    PROJECT_ROOT / "results" / "mc_dropout" / "efficientnetb4_mc_dropout_results.csv",
    index=False
)

print(
    "\nSaved image-level MC Dropout results to "
    "'efficientnetb4_mc_dropout_results.csv'."
)